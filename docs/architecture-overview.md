...
| **Julia Sprint 2 — Persistent runtime and nodes** | ✅ with conditions | `jarvis/julia/runtime/` owns the headless lifecycle supervisor, durable node registry, stable machine-local identity, execution/idempotency ledger, recovery dispositions, macOS LaunchAgent, portability export, and CLI. Node eligibility precedes Sprint 1 worker scoring. Diagnostics are exposed under `/api/julia/runtime/*`. Physical host-reboot evidence remains a validation condition. ADR-0037. |
